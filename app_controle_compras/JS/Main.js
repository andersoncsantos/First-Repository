  ...
  function setList(lista) {
      ...
      for (var key in lista) {
          table += '<tr><td>' + formatDesc(lista[key].desc) + '</td><td>' + formatAmount(lista[key].amount) + '</td><td>' + formatValue(lista[key].value) + '</td><td><button onclick="setUpdate(' + key + ')" class="btn btn-default">Edit</button> <button onclick="deleteData(' + key + ')" class="btn btn-default">Delete</button> </td></tr>';
      }
      table += '</tbody>';
